    ...
    public Uni<Void> produceDBWriteEvent(DBWriteRequest request) {
        ...
        return Uni.createFrom().emitter(em -> {
            Message<DBWriteRequest> message = Message.of(request)
                    ...
                    .withAck(() -> {
                        em.complete(null);
                        LOG.debugf("Successfully sent accounting DB create event for session: %s, %d ms", request.getSessionId(),System.currentTimeMillis()-startTime);
    ...
    public Uni<Void> produceAccountingResponseEvent(AccountingResponseEvent event) {
        ...
        return Uni.createFrom().emitter(em -> {
            Message<AccountingResponseEvent> message = Message.of(event)
                    ...
                    .withAck(() -> {
                        em.complete(null);
                        LOG.debugf("Successfully sent accounting response event for session: %s, %d ms", event.sessionId(),System.currentTimeMillis()-startTime);
                        return CompletableFuture.completedFuture(null);
                    })
                    .withNack(throwable -> {
                        LOG.errorf("Send failed: %s", throwable.getMessage());
    ...
    public Uni<Void> produceAccountingCDREvent(AccountingCDREvent event) {
        ...
        return Uni.createFrom().emitter(em -> {
            Message<AccountingCDREvent> message = Message.of(event)
                    ...
                    .withAck(() -> {
                        em.complete(null);
                        LOG.debugf("Successfully sent accounting CDR event for session: %s, %d ms", event.getPayload().getSession().getSessionId(),System.currentTimeMillis()-startTime);
                        return CompletableFuture.completedFuture(null);
                    })
                    .withNack(throwable -> {
    ...
    public Uni<Void> produceQuotaNotificationEvent(QuotaNotificationEvent event) {
        ...
        return Uni.createFrom().emitter(em -> {
            Message<QuotaNotificationEvent> message = Message.of(event)
                    ...
                    .withAck(() -> {
                        em.complete(null);
                        LOG.debugf("Successfully sent quota notification event for user: %s, %d ms",
